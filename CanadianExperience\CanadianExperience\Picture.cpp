...
/**
* Tree Dialog 1 set up
*/
void CPicture::TreeSeedDlg1()
{
	mTree1->SeedDlg();
	auto at = GetAnimationTime();
	SetAnimationTime(0);
	SetAnimationTime(at);
	UpdateObservers();
}
...
/**
* Tree Dialog 2 set up
*/
void CPicture::TreeSeedDlg2()
{
	mTree2->SeedDlg();
	auto at = GetAnimationTime();
	SetAnimationTime(0);
...
/**
* Harvest/grow the trees
*/
void CPicture::HarvestGrowTreeDlg()
{
	CTreestuffDlg dlg;
	dlg.SetVariables(mTree1, mTree2, mBasket);
	if (dlg.DoModal() == IDOK)
	{
		dlg.Take();
	}
...
void CPicture::Save(const std::wstring &filename)
{
    ...
    //
	root->SetAttribute(L"tree1start", mTree1->GetGrow());
	root->SetAttribute(L"tree2start", mTree2->GetGrow());
	root->SetAttribute(L"harvest", mBasket->GetHarvest());
	root->SetAttribute(L"empty", mBasket->GetEmpty());
...
void CPicture::Load(const std::wstring &filename)
{
    ...
    try
    {
        ...
        //
		mTree1->SetGrow(root->GetAttributeIntValue(L"tree1start", 0));
		mTree2->SetGrow(root->GetAttributeIntValue(L"tree2start", 0));
		mBasket->SetHarvest(root->GetAttributeIntValue(L"harvest", 250));
		mBasket->SetEmpty(root->GetAttributeIntValue(L"empty", 300));
